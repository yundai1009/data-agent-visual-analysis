...
# 文件头 · 数据画像（面试讲解）
# ------------------------------------------------------------
# 管什么：给任意 DataFrame 生成一份"体检报告"——字段分类
#   （数值/日期/分类/文本）、缺失统计、数据质量评级（A/B/C）、
#   自动洞察（异常值/相关性/分布集中度）、字段使用建议。
# 为什么需要它：LLM 无法直接"看"整张表，上传后必须先把它
#   压缩成结构化摘要（画像）。后续 Agent 选字段、推荐图表、
#   出结论全部基于这份画像，前端也用同份画像做字段推荐展示。
# 关键设计：
#   - _字段分组 是核心分诊：数值直判；日期用 50 行采样 + 80%
#     可解析率判断（不全表转换，省时）；短枚举（去重数 ≤30 且
#     平均长度 ≤6）归分类，长文本归文本；
#   - _生成数据质量 按缺失率/重复行/常量字段评级，超大表用
#     5 万行采样统计（50MB 级数据全表 duplicated 很慢）；
#   - _生成自动洞察 输出人类可读的洞察条目，直接进 LLM 上下文。
# 删除它会怎样：上传、预览、图表推荐、LLM 决策全部失去依据。
# ============================================================
