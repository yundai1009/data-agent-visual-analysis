...
def _占比数值字段(画像: Dict[str, Any], 需求文本: str) -> Optional[str]:
    """占比图的数值（y 轴）选择：需求提到数值字段 → 精确/泛化匹配；否则回退
    "价/额/量/值"类数值字段（阶段 48 修复：中文字段名股票数据，需求只说
    "按股票占比"时图表标签显示股票代码、数值却变成出现次数）。

    返回：数值字段名 str；找不到返回 None（调用方回退记录数计数兜底）。
    """
    数值字段 = 画像.get("数值字段") or []
    if not 数值字段:
        return None
    ...
    if 命中:
        return 命中
    # 回退：优先"价格/金额/数量"语义的字段（含 价/额/量/值/收益/利润/收入/市值/涨）
    数值候选 = [
        f for f in 数值字段
        if any(k in f for k in ("价", "额", "量", "值", "金额", "收益", "利润", "收入", "市值", "涨"))
    ]
    if 数值候选:
        return 数值候选[0]
    return None
...
def _受控语句配置(画像: Dict[str, Any], 分析需求: str) -> Dict[str, Any]:
    ...
    if "占比" in 文本 or "比例" in 文本 or "构成" in 文本:
        ...
        # 阶段 48：需求未提数值字段时（如"按股票占比"）优先回退"价/额/量/值"类字段，
        # 避免图表标签是股票代码、数值却变成出现次数。
