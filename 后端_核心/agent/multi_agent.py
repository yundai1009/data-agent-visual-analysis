...
from 后端_核心.agent.llm_client import chat_completion, extract_tool_call, is_llm_configured, embed_text
from 后端_核心.agent.tools import TOOL_SCHEMAS_FULL, execute_tool
...
def 多智能体分析(
    ...
    for attempt in range(max_retries + 1):
        ...
        # 【Bug20 修复】旧逻辑只用黑名单（不含"不通过/打回"即判通过）→ 审查员
        # 返回"建议优化：图表更适合用折线"等整改意见时被误判通过，打回重做闭环
        # 永远不可达。改为：白名单（"通过/没问题/合理"）+ 黑名单（"不通过/打回/
        # 建议修改/建议优化/调整"）组合判断——有整改意图即视为不通过，触发重试。
        摘要 = quality_result.get("摘要", "")
        _negative = any(k in 摘要 for k in ("不通过", "打回", "建议修改", "建议优化", "需调整", "不建议"))
        _positive = any(k in 摘要 for k in ("通过", "没问题", "合理", "可以"))
        passed = (not _negative) and (_positive or bool(摘要.strip()))
        if passed:
...
def _运行_agent(
    ...
    if not 需要工具:
        from 后端_核心.agent.llm_client import chat_completion as _纯文本
    ...
    for i in range(max_rounds):
        轮次 = 轮次起始 + i
        from 后端_核心.agent.orchestrator import _执行一轮 as _一轮
...
def _从消息提取意图(messages: List[Dict[str, Any]], 画像: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
    """从多轮消息中提取结构化意图。"""
    from 后端_核心.agent.orchestrator import _从消息提取意图 as _提取
