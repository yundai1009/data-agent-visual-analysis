...
- 所在目录：后端_核心/agent/
- 被谁调用：
  · 编排器.py → TraceRecorder（每轮 LLM/工具调用/观察都记录一条真实证据）
  · 多智能体.py → TraceRecorder（多智能体流程同样用它记录决策过程）
- 调用了谁：无第三方依赖，只用标准库 time
- 本文件负责：
  1. TraceRecorder：按轮次记录「LLM推理 / 工具调用 / 观察」三类步骤的证据
  2. 记录内容全部脱敏：不携带原始数据行，工具入参对敏感 key 打码
  3. 输出标准化 list[dict]，塞进响应里的 Agent_Trace 字段供前端渲染
  4. 附带 计时() 上下文管理器和 提取token() 小工具
- 面试要点：Trace 是"可审计性"的体现——用户能看见 Agent 每一步做了什么决策、
  花了多少 token/毫秒、成功还是失败；长度上限保证大表聚合也不会撑爆响应。
"""
...
class TraceRecorder:
    ...
    作用：编排器每执行一轮（LLM 调用/工具执行/观察）就调用对应方法记录一条，
    最后 to_list() 输出标准化证据链，前端可逐步渲染 Agent 的推理过程。

    on_event: 可选回调，每记录一条就实时推送一条
        ``{"type": "step", "data": record}``（供 SSE 直播使用）。回调异常
        被吞掉，绝不影响主流程。

    业务定位：可审计性的载体——让用户看到"Agent 为什么这么分析"，
    而不是黑盒给一个结果。
    """
