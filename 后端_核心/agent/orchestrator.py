...
from 后端_核心.agent.llm_client import (
    chat_completion,
    extract_tool_call,
    is_llm_configured,
    最近LLM失败,
...
from 后端_核心.agent.tools import (
    TOOL_SCHEMAS_FULL,
    execute_tool,
    validate_intent_against_profile,
)
from 后端_核心.agent.trace import TraceRecorder, 计时, 提取token
from 后端_核心.agent.executors import 注册所有执行器
from 后端_核心.agent.memory import 检索相似记忆, 保存记忆, 生成_few_shot_prompt, 记忆条数
...
# 阶段 35：意图路由快速判断
# ============================================================
# 模式表：(关键词组, 图表类型, 默认聚合, 推荐理由)
# 置信度：这些模式规则准确率 >90%，跳过 LLM 直接走规则，
# 省 3-4s、准确率更高；模糊需求仍进 LLM（分母变小）。
_快速模式表 = [
    ({"占比", "比例", "构成", "分布"}, "饼图", "计数",
     "分类占比——规则直接命中，无需 LLM"),
    ({"趋势", "随时间", "月份变化", "同比增长", "环比增长"}, "折线图", "求和",
     "时间趋势——时间轴明确，规则直接命中"),
    ({"Top", "top", "最高", "最低", "排行", "排名"}, "柱状图", None,
     "排名场景——柱状图+TopN（TopN 由上传报表生成器追加）"),
    ({"相关", "相关性", "关系", "散点"}, "散点图", None,
     "字段关系——散点图（两个数值列）"),
]


def _快速意图判断(分析需求: str, 画像: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """高置信度快速路由：关键词命中明确模式时直接返回规则结果，跳过 LLM。

    为什么在 _解析自然语言需求 里加这层：
    - 占比/趋势/TopN/相关性的规则准确率 >90%，LLM 多做一次调用只增加延迟（3-4s）不增加正确率；
    - 跳过后 LLM 只处理真正模糊的需求（"看看数据怎么样"），准确率分母变小、整体体感更好。
    为什么不合并进 _意图驱动配置：这里是"跳过 LLM"的快速通道，与 _意图驱动配置
    （规则兜底，LLM 失败时才用）在语义上不同——这里是"确定不用 LLM"，不是"LLM 试过了"。
    """
    文本 = 分析需求.strip().lower()
    字段列表 = 画像.get("字段列表", [])
    ...
    for 关键词组, 图表类型, 默认聚合, 推荐理由 in _快速模式表:
        if any(kw in 文本 for kw in 关键词组):
            if 图表类型 == "饼图":
                x轴 = next((f for f in 字段列表 if f in 分类字段), 字段列表[0] if 字段列表 else None)
            elif 图表类型 == "折线图":
                x轴 = next((f for f in 字段列表 if f in 日期字段), None)
            elif 图表类型 == "散点图":
                x轴 = next((f for f in 字段列表 if f in 分类字段), 字段列表[0])
            else:
                x轴 = next((f for f in 字段列表 if f not in 数值字段), 字段列表[0])

            if not x轴:
                continue

            # 修复占比/比例场景：识别用户提到的数值字段做 y 轴（如"价格占比"→y=价格），
            ...
            if not round2_ok:
                ...
                if round3_ok:
                    ...
                    if intent_override:
                        ...
                        from 后端_核心.field_selector import 自动选字段  # 解耦：规则层独立模块（原延迟导入上传报表生成器）
                        ...
                        try:
                            ...
                            from 后端_核心.agent.memory import 清理记忆  # 批次4：容量上限
                            清理记忆(5000)
...
def _从消息提取意图(messages: List[Dict[str, Any]], 画像: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    from 后端_核心.agent.tools import 归一化字段 as _归一化字段
