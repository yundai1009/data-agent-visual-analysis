...
# 文件头 · 报表生成/管理接口（面试讲解）
# ------------------------------------------------------------
# 管什么：全项目最核心的接口域——"用户提需求 → Agent 分析数据 →
#   产出图表+结论+风险提示"的全链路 HTTP 层：
#   - POST /reports/generate          同步生成（一次等完返回整份报表）
#   - POST /reports/generate-stream   SSE 直播（实时推送 Agent 决策事件）
#   - GET  /reports/...               历史列表 / 单份详情 / 导出 xlsx·csv·pdf
#   - DELETE /reports/{id}            删除
#   - 分享链接（只读 + 可选密码）与"重放报表"（同一需求重跑）
# 关键设计（面试可讲）：
#   - 双路径：single（单 Agent 规则+LLM）与 multi（多智能体协作），
