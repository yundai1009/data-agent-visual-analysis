...
2. **怎么跑**：`.\启动\启动.ps1` 一键启动，浏览器自动打开 `http://127.0.0.1:5173`
3. **要不要 LLM key**：不配也能跑，自动降级到关键词匹配兜底；配了 DeepSeek/OpenAI key 用 LLM
4. **要不要 MySQL**：不要。SQLite 持久化，零外部服务，零网络依赖

---

## 🗺️ 零代码视角的目录地图（给完全没写过代码的人）

> 这一节不讲代码，只回答三个问题：**这个项目哪些文件干吗的、我先看哪几个、哪些永远不用碰**。
> 方法：打开项目文件夹，按下面的顺序从上往下点开看。

### 第 1 步：先看这 3 个（5 分钟看懂全貌）

| 文件/文件夹 | 它是什么（人话） | 要不要读 |
|------------|-----------------|---------|
| `README.md` | 就是你现在读的这份说明书 | ✅ 读完这页即可 |
| `docs/使用指南/` | 使用说明 + 启动指南 | ✅ 想用起来先看这 |
| `docs/开发日志/项目开发日志.md` | 项目"成长日记"：每个阶段为什么做、怎么取舍 | ✅ 想了解来龙去脉看这 |

### 第 2 步：知道每个文件夹的角色（不用读代码，知道分工即可）

```
项目根目录/
├── 启动/            ← 双击就能跑的东西（.bat/.ps1 启动脚本 + .lnk 快捷方式）
├── frontend/        ← 网页长什么样（React 前端，浏览器里看到的一切）
│   └── src/         ← 前端源码（页面、组件、发请求的代码都在里面）
├── api/             ← 后端"收快递"的地方（浏览器请求先到这里，再转给干活的人）
├── services/        ← 后端"跑腿"的服务（登录验证、发邮件、定时任务）
├── repositories/    ← 后端"记账本"（把数据存进 SQLite 数据库）
├── 后端_核心/        ← 后端"大脑"（Agent 推理、数据画像、生成报表的核心逻辑）
│   └── agent/       ← Agent 智能体（让 AI 思考/调工具的编排器、LLM 客户端）
├── data/            ← 数据文件存放处（上传的文件、数据库、导出结果）
├── tests/           ← 自动化测试（代码的"体检报告"，验证没改坏东西）
├── docs/            ← 各种文档（使用指南、开发日志、工程文档）
└── scripts/         ← 运维/评测小工具（备份数据库、跑评测脚本）
```

### 第 3 步：这个项目怎么"转"起来的（一次请求的旅程）

```
你在网页输入"按地区统计销售额占比"
   ↓
frontend/src/pages/Analysis.jsx   ← 网页把这句话交给前端
   ↓
frontend/src/api/endpoints.js     ← 前端打包成请求，发给后端
   ↓
api/routes/reports.py             ← 后端收到请求
   ↓
后端_核心/agent/orchestrator.py   ← Agent 大脑：理解意图（LLM 或关键词规则）
   ↓
后端_核心/agent/tools.py          ← Agent 调工具：选字段、聚合计算
   ↓
后端_核心/report_generator.py     ← 生成图表配置 + 结论
   ↓
后端把结果存进 data/daa.db        ← 记账
   ↓
前端 EChartsChart.jsx             ← 画成图表显示给你看
```

### 第 4 步：这些文件/文件夹 **永远不用碰**（避免被吓到）

| 路径 | 为什么不用碰 |
|------|------------|
| `frontend/node_modules/` | 前端依赖包（几十万个自动装的库文件） |
| `frontend/dist/` `frontend/dist-demo/` | 构建产物（代码打包后的结果，`npm run build` 自动生成） |
| `data/uploads/` `data/export/` `data/backups/` | 运行时数据（上传文件、导出结果、备份） |
| `data/daa.db` `data/demo.db` | 数据库文件（SQLite，程序自动读写） |
| `.git/` `.reasonix/` `.pytest_cache/` `__pycache__/` | 版本历史 / 工具缓存（自动生成） |
| `后端_核心/*.py` 里中文名 shim 文件（如 `数据画像.py`、`编排器.py`） | 这些是"改名过渡壳"（一行转发代码），逻辑在对应的英文名文件（如 `profile.py`、`orchestrator.py`）里 |

### 第 5 步：新手建议的阅读顺序（想从代码里学点什么）

1. `README.md` → `docs/使用指南/`（先会用）
2. `docs/工程文档/从零理解项目.md`（理解设计，本地文档，可问作者要）
3. 只读 `api/routes/reports.py` + `frontend/src/pages/Analysis.jsx`（看一条请求从浏览器到报表的完整链路）
4. 其余文件**看完这一条链路前不要碰**——它们是这条链路上各环节的"零件"，看零件前先懂整车
...
- **条件筛选 + TopN**："只看华东区"/"销量 Top 10"——筛选贯穿画像/聚合/结论全链路，无 LLM key 也能听懂（规则层兜底）
- **同比环比**：日期时间序列图自动计算环比/同比（差值 + 百分比）
- **多轮追问**：生成后可连续追问（"那华南区呢？""按月份对比呢？"），报表带追问溯源链
- **图表看板**：多份报表并排对比，可命名保存 / 排序 / 移除
- **报表分享**：带时效与可选密码的公开只读链接
- **导出全家桶**：Excel / CSV / PDF / 图表 PNG / Agent 决策记录；PDF 支持**图文完整报告**（图表图 + 结论 + 数据表 + Trace）
- **报表模板 + 定时生成**：分析配置一键收藏为模板，模板 + cron 到点自动出报表（如每周一早 9 点）
- **深色模式 + 新手引导**：一键深色（全站适配、偏好记忆）；首次登录 3 步引导上手
...
| 维度 | 落地 |
|------|------|
| 质量 | 171 后端测试 + 29 前端单测 + 服务级/真实浏览器双冒烟；**CI（GitHub Actions）每次 push 全量回归** |
...
**推荐方式：双击桌面/项目目录的启动图标（无黑框、无窗口）**

| 图标 | 用途 |
|---|---|
| `启动数据分析平台.lnk` | 正式模式：静默起后端 → 浏览器打开品牌启动页 → 自动跳转登录页 |
| `启动数据分析平台-演示.lnk` | 演示模式：免登录 + 自动加载示例数据 |
| `停止服务.lnk`（或 `启动\停止服务.bat`） | 停止后台服务 |

体验流程：双击 → 后端后台静默启动 → 浏览器自动打开启动页（地球夜景动画 + Logo）→ 启动页实时探测后端就绪 → 自动跳转登录页 / 首页。
...
| POST | `/reports/generate` | 用自然语言 + 字段配置生成可视化报表（支持筛选条件/TopN/对比） |
| POST | `/reports/{id}/export-report` | 完整 PDF 报告导出（图表图 + 结论 + 数据表 + Trace，前端传图表 PNG） |
| POST | `/templates` | 保存报表模板（分析配置收藏）；GET 列表 / DELETE 删除 / POST /{id}/run 一键执行 |
| POST | `/schedules` | 创建定时任务（模板 + cron）；GET 列表 / DELETE 删除 |
...
不依赖网络、不依赖真实 LLM key，后端 171 个测试 + 前端 29 个单测可在秒级跑完；CI（GitHub Actions）每次 push 自动跑 后端 pytest + 前端 build/lint/单测 + 服务级与真实浏览器双冒烟。
...
| 8000 端口被占 | `.\启动\启动.ps1 -BackendPort 8001` |
| 8501 端口被占 | `.\启动\启动.ps1 -FrontendPort 8502` |
