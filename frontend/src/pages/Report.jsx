...
import { Download, DownloadCloud, Sparkles, Clock, Loader2 } from 'lucide-react';
...
export default function Report() {
  ...
  const chartType = report.图表类型 || '柱状图';
  const intentSource = report.意图来源 || 'AI';
  const title = report.标题 || '分析报表';
  const exportData = report.导出数据 || {};

  // Build chart bars from actual data
  const xField = chartConfig.X轴 || (chartData.length > 0 ? Object.keys(chartData[0])[0] : '');
  const yField = (chartConfig.Y轴 || [])[0] || (chartData.length > 0 ? Object.keys(chartData[0])[1] : '');
  const maxValue = Math.max(...chartData.map((d) => Number(d[yField]) || 0), 1);
    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-7">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="px-2.5 py-1 rounded text-xs bg-gray-50 text-gray-500 border border-gray-200">{chartType}</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-gray-400">{xField} 分布</span>
        </div>
        {chartData.length > 0 ? (
          <div className="flex items-end justify-around h-56 gap-3 px-2">
            {chartData.slice(0, 12).map((row, i) => {
              const val = Number(row[yField]) || 0;
              const pct = Math.max((val / maxValue) * 100, 2);
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
                  <span className="text-xs text-gray-400 font-mono">{val.toLocaleString()}</span>
                  <div
                    className="w-full rounded-t-lg bg-gradient-to-t from-indigo-500 to-indigo-400 hover:from-indigo-600 transition-all cursor-pointer"
                    style={{ height: `${pct}%` }}
                  />
                  <span className="text-xs text-gray-500 truncate max-w-full">{row[xField]}</span>
                </div>
              );
            })}
          </div>
        ) : (
          <div className="h-56 flex items-center justify-center text-gray-400 text-sm">暂无图表数据</div>
        )}
      </div>
